...
**多源内容智能处理器：任意内容 → 内容诊断 / NotebookLM 播客包 / PPT 设计稿 / PPT+播客动态视频 / 微信公众号草稿 / 小红书科普帖**

把文章、剪藏、网页、播客转录稿或长文资料，先做内容价值与传播价值诊断，再整理成可交付的播客音频包、PPT 设计提示词/幻灯片稿、PPT+播客动态视频，并按媒体写作逻辑改写成微信公众号草稿或小红书发布稿。
...
2. **NotebookLM 播客生成**：创建或复用 NotebookLM 笔记本，添加 source，按诊断策略生成 audio overview，并下载音频。
3. **播客包装**：用诊断策略生成播客标题、简介和 16:9 封面图。
4. **媒体写作分析**：拆解原文标题、开头、结构、受众、承诺、案例和可复用洞察。
5. **公众号重写**：基于原始内容和诊断结果重构角度、结构、语气和标题，生成新的微信长文。
6. **微信草稿上传**：转换 Markdown 为 WeChat HTML，处理图片和封面，调用 `md2wechat` 创建微信公众号草稿。
7. **PPT 设计提示词生成**：按页输出理解与决策结果、中文版提示词、英文版提示词，并约束为高级极简初创企业审美。
8. **PPT+播客动态视频**：通过 HyperFrames 生成强叙事动态视频，按音频时间轴播放幻灯片，叠加完整字幕和关键词强调。
9. **小红书科普帖生成**：生成可复制发布的 Markdown 与结构化 JSON，包含帖子类型、标题候选、解释型正文、标签和封面建议。
