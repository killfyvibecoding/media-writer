...
def resolve_tokens_file():
    configured = os.environ.get('GETNOTE_TOKENS_FILE')
    candidates = [
        configured,
        '~/.codex/skills/getnote/tokens.json',
        '~/.claude/skills/getnote/tokens.json',
    ]
    for candidate in candidates:
        if not candidate:
            continue
        expanded = os.path.expanduser(candidate)
        if configured or os.path.exists(expanded):
            return expanded
    return os.path.expanduser('~/.codex/skills/getnote/tokens.json')
